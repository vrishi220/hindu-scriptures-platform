...
export async function GET(
  ...
  const doGet = (token?: string) =>
    fetch(`${API_BASE_URL}/api/edition-snapshots/${id}/render-artifact`, {
      headers: {
  ...
  if (!response.ok) {
    return NextResponse.json(payload || { detail: "Failed to fetch render artifact" }, { status: response.status });
  }
