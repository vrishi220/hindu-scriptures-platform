...
export async function DELETE(
  ...
  const { bookId } = await params;
  const store = await cookies();
  const target = new URL(`/api/content/books/${encodeURIComponent(bookId)}`, API_BASE_URL);
  ...
  let response: Response;
  try {
    response = await fetch(target.toString(), { method: "DELETE", headers, cache: "no-store" });
  } catch {
    return NextResponse.json(
  ...
  const payload = await response.json().catch(() => null);
  if (!response.ok) {
    return NextResponse.json(payload || { detail: "Book deletion failed" }, { status: response.status });
  }
  return NextResponse.json(payload);
}
