...
export async function GET() {
  ...
  const doGet = (token?: string) =>
    fetch(`${API_BASE_URL}/api/cart/me`, {
      headers: {
  ...
  if (!response.ok) {
    return NextResponse.json(payload || { detail: "Failed to load cart" }, { status: response.status });
  }
...
export async function DELETE() {
  ...
  const payload = await response.json().catch(() => null);
  if (!response.ok) {
    return NextResponse.json(payload || { detail: "Failed to clear cart" }, { status: response.status });
  }
