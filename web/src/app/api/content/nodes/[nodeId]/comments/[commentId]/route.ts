...
export async function PATCH(
  ...
  const doPatch = (token?: string) =>
    fetch(`${API_BASE_URL}/api/content/nodes/${resolvedParams.nodeId}/comments/${resolvedParams.commentId}`, {
      method: "PATCH",
      headers: {
    ...
    if (refreshed?.access_token) {
      ...
      if (!response.ok) {
        return NextResponse.json(
          payload || { detail: "Failed to update comment" },
...
export async function DELETE(
  ...
  if (response.status === 401 && refreshToken) {
    ...
    if (refreshed?.access_token) {
      ...
      if (!response.ok) {
        return NextResponse.json(
          payload || { detail: "Failed to delete comment" },
          { status: response.status }
        );
      }
      return res;
