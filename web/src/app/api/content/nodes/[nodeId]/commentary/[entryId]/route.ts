...
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ nodeId: string; entryId: string }> }
) {
  ...
  const doPatch = (token?: string) =>
    fetch(`${API_BASE_URL}/api/content/nodes/${resolvedParams.nodeId}/commentary/${resolvedParams.entryId}`, {
      method: "PATCH",
      headers: {
    ...
    if (refreshed?.access_token) {
      ...
      if (!response.ok) {
        return NextResponse.json(
          payload || { detail: "Failed to update commentary" },
...
export async function DELETE(
  ...
  if (response.status === 401 && refreshToken) {
    ...
    if (refreshed?.access_token) {
      ...
      if (!response.ok) {
        return NextResponse.json(
          payload || { detail: "Failed to delete commentary" },
          { status: response.status }
        );
      }
      return res;
