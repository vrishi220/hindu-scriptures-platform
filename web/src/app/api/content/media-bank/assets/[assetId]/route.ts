...
export async function PATCH(
  ...
  const doPatch = (token?: string) =>
    fetch(`${API_BASE_URL}/api/content/media-bank/assets/${resolvedParams.assetId}`, {
      method: "PATCH",
      headers: {
    ...
    if (refreshed?.access_token) {
      ...
      if (!response.ok) {
        return NextResponse.json(
          payload || { detail: "Failed to update media asset" },
...
export async function DELETE(
  ...
  if (response.status === 401 && refreshToken) {
    ...
    if (refreshed?.access_token) {
      ...
      if (!response.ok) {
        return NextResponse.json(
          payload || { detail: "Failed to delete media asset" },
          { status: response.status }
        );
      }
      return res;
